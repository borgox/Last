...
if not os.path.exists(venv_path):
    os.system("python -m venv .venv") if os.name == "nt" else os.system("python3 -m venv .venv")
...
if not os.getenv("VIRTUAL_ENV"):
    ...
    if os.name == 'nt':
        ...
        args = ' '.join(shlex.quote(arg) for arg in sys.argv)
        subprocess.call(f"source {activate_script} && python {args}", shell=True, executable="/bin/bash")
        sys.exit()
import random
import sys
import time
...
class Logger:
    ...
    def log(message: str, status: str, status_color: str, end: str = "\n") -> None:
        ...
        print(Logger.center_with_ansi(full_msg, terminal_width) + Style.RESET_ALL, end=end)
...
class MultiTool:
    def __init__(self):
        ...
        self.spotify = Spotify(client_credentials_manager=SpotifyClientCredentials(client_id=SPOTIFY_CLIENT_ID, client_secret=SPOTIFY_CLIENT_SECRET) ,auth_manager=self.spotify_oauth)
    ...
    def gradient_progress_bar(self, count, total, start_time):
        ...
        max_bar_width = terminal_width - (time_width + percentage_width + brackets_width + spacing_width)
        ...
        ctypes.windll.kernel32.SetConsoleTitleW(f"@Last | {count}/{total} | {elapsed_str} | Made by @borghetoo")
    ...
    def multiple_scrobbles(self, artist, title, count=1, **kwargs):
        ...
        t = threading.Thread(target=self.run, args=(artist, title, count), kwargs=kwargs)
        t.start()
    ...
    def interactive_mode(self, count=1):
        spotify_url = Logger.input_prompt("Enter Spotify Track URL (leave blank for manual entry)")
        if spotify_url:
    ...
    def parse_arguments(self):
        ...
        parser.add_argument("--album_artist", type=str, help="Album artist name", default=None)
        parser.add_argument("--track_number", type=int, help="Track number", default=None)
        ...
        if args.spotify:
            ...
            self.multiple_scrobbles(
                artist, title, count=args.count,
                album=args.album, album_artist=args.album_artist,
                track_number=args.track_number, duration=args.duration,
                stream_id=args.stream_id, context=args.context, mbid=args.mbid
            )
        elif args.artist and args.title:
            self.multiple_scrobbles(
                args.artist, args.title, count=args.count,
                album=args.album, album_artist=args.album_artist,
                track_number=args.track_number, duration=args.duration,
                stream_id=args.stream_id, context=args.context, mbid=args.mbid
            )
